...
namespace StbImageSharp
{
	...
	{
		internal static int stbi__bmp_test(stbi__context s)
		{
		...
		internal static void* stbi__bmp_load(stbi__context s, int* x, int* y, int* comp, int req_comp,
		...
		internal static int stbi__bmp_info(stbi__context s, int* x, int* y, int* comp)
		{
			void* p;
			var info = new stbi__bmp_data();
			info.all_a = 255;
		...
		internal static int stbi__bmp_test_raw(stbi__context s)
		{
			var r = 0;
			var sz = 0;
			if (stbi__get8(s) != 66)
		...
		internal static int stbi__bmp_set_mask_defaults(stbi__bmp_data* info, int compress)
		{
			if (compress == 3)
				return 1;
			if (compress == 0)
		...
		internal static void* stbi__bmp_parse_header(stbi__context s, stbi__bmp_data* info)
		{
			var hsz = 0;
		...
		internal struct stbi__bmp_data
		{
			internal int bpp;
			internal int offset;
			internal int hsz;
