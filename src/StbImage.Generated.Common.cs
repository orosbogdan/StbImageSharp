...
namespace StbImageSharp
{
	unsafe partial class StbImage
	{
		...
		internal const int STBI_ORDER_RGB = 0;
		internal const int STBI_ORDER_BGR = 1;
		internal const int STBI__SCAN_load = 0;
		internal const int STBI__SCAN_type = 1;
		internal const int STBI__SCAN_header = 2;
		internal static int stbi__vertically_flip_on_load_global;
		[ThreadStatic]
		internal static int stbi__vertically_flip_on_load_local;
		[ThreadStatic]
		internal static int stbi__vertically_flip_on_load_set;
		internal static float stbi__l2h_gamma = 2.2f;
		internal static float stbi__l2h_scale = 1.0f;
		internal static float stbi__h2l_gamma_i = 1.0f / 2.2f;
		internal static float stbi__h2l_scale_i = 1.0f;
		internal static int stbi__unpremultiply_on_load_global;
		internal static int stbi__de_iphone_flag_global;
		[ThreadStatic]
		internal static int stbi__unpremultiply_on_load_local;
		[ThreadStatic]
		internal static int stbi__unpremultiply_on_load_set;
		[ThreadStatic]
		internal static int stbi__de_iphone_flag_local;
		[ThreadStatic]
		internal static int stbi__de_iphone_flag_set;
		internal static byte[] stbi__process_marker_tag = { 65, 100, 111, 98, 101, 0 };
		internal static byte[] stbi__process_frame_header_rgb = { 82, 71, 66 };

		internal static byte[] stbi__compute_huffman_codes_length_dezigzag =
			{ 16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15 };

		internal static int[] stbi__shiftsigned_mul_table = { 0, 0xff, 0x55, 0x49, 0x11, 0x21, 0x41, 0x81, 0x01 };
		internal static int[] stbi__shiftsigned_shift_table = { 0, 0, 0, 1, 0, 2, 4, 6, 0 };
		...
		internal static void* stbi__malloc(ulong size)
		{
			return CRuntime.malloc(size);
		}

		internal static int stbi__addsizes_valid(int a, int b)
		{
			if (b < 0)
				return 0;
		...
		internal static int stbi__mul2sizes_valid(int a, int b)
		{
			if (a < 0 || b < 0)
				return 0;
			if (b == 0)
		...
		internal static int stbi__mad2sizes_valid(int a, int b, int add)
		{
			return stbi__mul2sizes_valid(a, b) != 0 && stbi__addsizes_valid(a * b, add) != 0 ? 1 : 0;
		}

		internal static int stbi__mad3sizes_valid(int a, int b, int c, int add)
		{
		...
		internal static int stbi__mad4sizes_valid(int a, int b, int c, int d, int add)
		{
			return stbi__mul2sizes_valid(a, b) != 0 && stbi__mul2sizes_valid(a * b, c) != 0 &&
		...
		internal static void* stbi__malloc_mad2(int a, int b, int add)
		{
		...
		internal static void* stbi__malloc_mad3(int a, int b, int c, int add)
		{
		...
		internal static void* stbi__malloc_mad4(int a, int b, int c, int d, int add)
		{
		...
		internal static int stbi__addints_valid(int a, int b)
		{
			if (a >= 0 != b >= 0)
				return 1;
			if (a < 0 && b < 0)
		...
		internal static int stbi__mul2shorts_valid(int a, int b)
		{
			if (b == 0 || b == -1)
				return 1;
			if (a >= 0 == b >= 0)
		...
		internal static float* stbi__ldr_to_hdr(byte* data, int x, int y, int comp)
		{
			var i = 0;
			var k = 0;
			var n = 0;
		...
		internal static void* stbi__load_main(stbi__context s, int* x, int* y, int* comp, int req_comp,
		...
		internal static byte* stbi__convert_16_to_8(ushort* orig, int w, int h, int channels)
		{
			var i = 0;
		...
		internal static ushort* stbi__convert_8_to_16(byte* orig, int w, int h, int channels)
		{
			var i = 0;
			var img_len = w * h * channels;
			ushort* enlarged;
		...
		internal static void stbi__vertical_flip(void* image, int w, int h, int bytes_per_pixel)
		{
			var row = 0;
		...
		internal static void stbi__vertical_flip_slices(void* image, int w, int h, int z, int bytes_per_pixel)
		{
			var slice = 0;
		...
		internal static byte* stbi__load_and_postprocess_8bit(stbi__context s, int* x, int* y, int* comp, int req_comp)
		{
		...
		internal static ushort* stbi__load_and_postprocess_16bit(stbi__context s, int* x, int* y, int* comp, int req_comp)
		{
			var ri = new stbi__result_info();
		...
		internal static void stbi__float_postprocess(float* result, int* x, int* y, int* comp, int req_comp)
		{
			if ((stbi__vertically_flip_on_load_set != 0
				? stbi__vertically_flip_on_load_local
		...
		internal static float* stbi__loadf_main(stbi__context s, int* x, int* y, int* comp, int req_comp)
		{
			byte* data;
			if (stbi__hdr_test(s) != 0)
			{
		...
		internal static int stbi__get16be(stbi__context s)
		{
			int z = stbi__get8(s);
			return (z << 8) + stbi__get8(s);
		}

		internal static uint stbi__get32be(stbi__context s)
		{
			var z = (uint)stbi__get16be(s);
			return (uint)((z << 16) + stbi__get16be(s));
		}

		internal static int stbi__get16le(stbi__context s)
		{
			int z = stbi__get8(s);
			return z + (stbi__get8(s) << 8);
		}

		internal static uint stbi__get32le(stbi__context s)
		{
		...
		internal static byte stbi__compute_y(int r, int g, int b)
		{
			return (byte)((r * 77 + g * 150 + 29 * b) >> 8);
		}

		internal static byte* stbi__convert_format(byte* data, int img_n, int req_comp, uint x, uint y)
		{
			var i = 0;
			var j = 0;
			byte* good;
		...
		internal static ushort stbi__compute_y_16(int r, int g, int b)
		{
			return (ushort)((r * 77 + g * 150 + 29 * b) >> 8);
		}

		internal static ushort* stbi__convert_format16(ushort* data, int img_n, int req_comp, uint x, uint y)
		{
			var i = 0;
			var j = 0;
			ushort* good;
		...
		internal static byte stbi__clamp(int x)
		{
			if ((uint)x > 255)
			{
				if (x < 0)
		...
		internal static byte stbi__blinn_8x8(byte x, byte y)
		{
			var t = (uint)(x * y + 128);
			return (byte)((t + (t >> 8)) >> 8);
		}

		internal static int stbi__bitreverse16(int n)
		{
		...
		internal static int stbi__bit_reverse(int v, int bits)
		{
			return stbi__bitreverse16(v) >> (16 - bits);
		}

		internal static int stbi__high_bit(uint z)
		{
			var n = 0;
			if (z == 0)
				return -1;
		...
		internal static int stbi__bitcount(uint a)
		{
		...
		internal static int stbi__shiftsigned(uint v, int shift, int bits)
		{
			if (shift < 0)
				v <<= -shift;
			else
		...
		internal static int stbi__info_main(stbi__context s, int* x, int* y, int* comp)
		{
		...
		internal static int stbi__is_16_main(stbi__context s)
		{
